...
import { hostName, key } from "../../config/config";

// get city data
export const getCityData = createAsyncThunk("city", async (obj) => {
  try {
    const request = await axios.get(
      `${hostName}/data/2.5/weather?q=${obj.city}&units=${obj.unit}&APPID=${key}`
    );
...
// get 5 days forecast of the entered city

export const getFiveDaysForecast = createAsyncThunk("5days", async (obj) => {
  const request = await axios.get(
    `${hostName}/data/2.5/forecast?lat=${obj.lat}&lon=${obj.lon}&units=${obj.unit}&APPID=${key}`
  );
...
const weatherSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getFiveDaysForecast.rejected, (state, action) => {
        state.forecastLoading = false;
        state.forecastData = null;
